...
- 四角形: DOM上の標準form送信境界
- 円形: `webRequest`で観測したfetch/XHR系通信
- 波形: `webRequest`で観測したBeacon/Ping系通信
- 中央文字: GET、POST等のmethod
- 右上の点: Cookieヘッダー名の検出状態
- 左上の短線: 別オリジン関係

Cookie点は、塗りが検出、空洞が未検出、破線が未観測または判定不能を示す。「未検出」はCookie不存在の保証ではない。

ネットワーク通信パルスはすべて本文未観測であり、本文あり／なしを推定しない。

### 28.5 色の非評価性

色は形を補助する分類情報としてだけ使い、安全、危険、警告、注意を表さない。赤・青・黄の信号的三色を基調とせず、低彩度の黒鉛、紫灰、灰緑、銅灰を用いる。

色が判別できない状態でも、外形、内部線、点、破線、method文字によって観測経路を識別できることを要件とする。
