...
## 28. v0.4.3実装差分：リアルタイム境界パルス可視化

v0.4.3では、観測ログを読む場面と、ページ上で通信境界の時間配置を感じ取る場面を接続する。

### 28.1 ログ画面からの観測範囲確認

ログ画面は設定画面と同一のCoverage Manifest生成関数を利用し、ダイアログで次を確認できる。
...
ログの解釈中に観測限界へ戻れることを要件とする。

### 28.2 観測表の上下横スクロール

広い観測表には、下側のネイティブ横スクロールに加え、上側の同期スクロール面を設ける。両者の`scrollLeft`を同期し、表幅の変化を`ResizeObserver`で反映する。

### 28.3 八方向配置

文章チップと通信パルスは、次の閉じた位置分類を共有する。
...
文章チップの移動ハンドルは時計回りに位置を切り替え、ローカル設定へ保存する。文章チップ本体は引き続き操作透過とする。

### 28.4 通信パルス

Level 2、Level 3、MAXでは、利用者設定により小型通信パルスを表示できる。Level 1標準モードでは表示しない。

通信パルスは、一件の安全・危険判定ではなく、観測経路と時間配置を示す。

- 外周形: HTTP method。GETは円、POSTは四角形、PUTはひし形、PATCHは六角形、DELETEは三角形とする。その他のmethodにも閉じた外形を割り当てる。
- 中央文字: 通信方式。Sは標準form submit、Fはfetch/XHR、BはBeacon/Pingを示す。
- 色: 観測経路。濃いマゼンタはDOM観測、淡いシアンは`webRequest`観測を示す。
- 右上記号: Cookieヘッダー名の検出状態。
- 左上の短線: 別オリジン関係。

Cookie記号は、●が検出、−が未検出、·が未観測、?が判定不能を示す。「未検出」はCookie不存在の保証ではない。

ネットワーク通信パルスはすべて本文未観測であり、本文あり／なしを推定しない。

### 28.5 色の非評価性

色は形と文字を補助する観測経路情報としてだけ使い、安全、危険、警告、注意を表さない。DOM観測には濃いマゼンタ、`webRequest`観測には淡いシアンを用いるが、両者を危険度または安全度へ対応づけない。

色が判別できない状態でも、外周形によるmethod、中央文字による通信方式、Cookie記号、別オリジン短線によって観測内容を識別できることを要件とする。
