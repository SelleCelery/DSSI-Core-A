...
入力欄へのフォーカス、キー入力、貼り付け、自動入力らしき更新、送信試行、同意チェック、外部遷移、ダウンロード試行を境界イベントとして扱う。ただし、境界イベントとして扱うことと通常ログへ保存することは同義ではない。v0.4.1ではフォーカスを一時チップだけに限定し、通常ログと通信相関から除外する。
...
信頼済みの内容変更後に同一タブ・同一フレームから対象通信が発生した場合、Core A は「内容変更と時間的に近接して通信が発生した」という事実を記録できる。フォーカスだけではこの相関を成立させない。

ただし、通信本文を観測していない限り、入力内容が漏えいした、保存された、送信されたと断定してはならない。

許容表現：

> 内容変更操作と近接して通信が発生しました。DSSIは、その通信に入力内容が含まれるかを確認できません。
...
任意通信観測を有効にした場合、Core AはChromeが提供したrequest-header集合について、`Cookie`というヘッダー名の検出状態だけを記録できる。
...
`not_detected`はCookie不存在の保証ではない。Cookie値、Cookie名、Cookie個数、認証用途は記録または推定しない。

### 9.6 ページ観測開始との時間関係

通信記録はDSSI自身のページ観測開始から5秒以内／5秒超／不明という中立的な時間関係を持つことができる。この項目から初期化、認証、状態復元、分析等の用途を推定してはならない。
