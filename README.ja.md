...
導入説明を確認したあと、標準設定、通信メタデータを観測しない設定、観測を開始しない休止状態の三つから選びます。標準設定では、任意の`webRequest`権限と、その観測に必要なHTTP/HTTPSホストアクセスを要求します。いつでも「設定とプライバシー」で選択を変更でき、通信メタデータ観測を外すと`webRequest`権限だけが撤回されます。ページ上の限定観測にも使うHTTP/HTTPS範囲は撤回対象に含めません。
