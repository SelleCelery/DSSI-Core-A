...
2. 初回起動時に、入力面チップ、通信パルス、観測ログの読み方を架空サイトで確認するチュートリアル。
3. 任意権限を付与する前の判断を支援する導入説明画面。

初回インストールではチュートリアルが開き、最終場面から導入説明と観測方法の選択へ進みます。途中で閉じた場合は最後の場面を端末内へ保存し、ポップアップまたは設定画面から続けられます。拡張機能が自動的に開き直すことはありません。
